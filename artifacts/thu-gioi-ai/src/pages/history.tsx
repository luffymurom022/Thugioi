  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-700">
      <header className="border-b border-primary/20 pb-6 sticky top-0 bg-background/80 backdrop-blur-md z-10 pt-4">
        ...
        <p className="text-primary/70 font-mono mt-2">NHẬT KÝ SỰ KIỆN THẾ GIỚI</p>
      </header>
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 relative">
            {history.map((entry) => {
              ...
              const isWar = entry.eventType === 'war' || entry.eventType === 'conquest' || entry.eventType === 'battle';
              ...
              return (
                <div key={entry.id} className="flex flex-col md:flex-row gap-4 md:gap-8 items-start group">
                  <div className="font-mono text-sm tracking-widest text-primary/60 shrink-0 md:w-20 pt-3 md:text-right flex items-center md:block gap-4">
                    <span className="md:hidden">NĂM</span> {entry.worldDay.toString().padStart(4, '0')}
                    <div className={`w-3 h-3 rounded-full md:absolute md:left-[95px] md:mt-1 border-2 border-background z-10 transition-colors ${
                      isExtinction ? 'bg-destructive' : 
                      isBirth ? 'bg-green-400' : 
                      isWar ? 'bg-red-500' :
                      'bg-primary group-hover:bg-secondary'
                    }`} />
                  </div>
                  
                  <div className={`flex-1 hologram-border bg-card/40 p-4 md:p-6 transition-colors hover:bg-card/60 ${isExtinction ? 'border-destructive/30' : isWar ? 'border-red-500/20' : ''}`}>
                    <div className="flex justify-between items-start mb-2">
                      <span className={`text-xs font-mono uppercase px-2 py-1 ${
                        ...
                        isWar ? 'bg-red-500/20 text-red-400' :
                        'bg-primary/20 text-primary'
                      }`}>
                        {EVENT_TYPE_VI[entry.eventType] ?? entry.eventType.replace('_', ' ').toUpperCase()}
                      </span>
            ...
            {history.length === 0 && (
              <div className="ml-12 md:ml-[140px] py-10 font-mono text-muted-foreground border border-dashed border-primary/20 bg-black/20 p-6 text-center">
                BIÊN NIÊN SỬ TRỐNG. CHƯA CÓ SỰ KIỆN NÀO ĐƯỢC GHI LẠI.
              </div>
            )}
          </div>
        )}
