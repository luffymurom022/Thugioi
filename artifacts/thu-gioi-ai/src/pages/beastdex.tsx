  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="border-b border-primary/20 pb-6">
        <h1 className="text-4xl font-bold tracking-widest text-foreground uppercase hologram-text mb-4">Bách Thú Đồ</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(creature => (
            <Link key={creature.id} href={`/beastdex/${creature.id}`} className="block group cursor-pointer" data-testid={`card-creature-${creature.id}`}>
                <div className={`hologram-border p-5 relative overflow-hidden transition-all duration-300 group-hover:bg-primary/5 group-hover:border-primary/80 ${creature.status === 'extinct' ? 'bg-destructive/5 border-destructive/30 grayscale' : 'bg-card/40'}`}>
                  ...
                  <div className="text-xs font-mono text-primary/60 mb-1">Mã. {creature.id.toString().padStart(4, '0')}</div>
                  ...
                  <div className="space-y-1.5 text-sm font-mono">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-foreground">{creature.rank}</span>
                    </div>
                    <div className="flex justify-between">
